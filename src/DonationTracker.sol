...
contract DonationTracker is Ownable {
    ...
    function submitReport(uint256 _fundId, uint256 _milestoneIndex, string memory _description, string memory _proofCID)
        external
    {
        require(bytes(_proofCID).length > 0, "Proof CID required");

        reports[_fundId][_milestoneIndex] = Report(_fundId, _milestoneIndex, _description, _proofCID, false);
    ...
    function getReportDetails(uint256 _fundId, uint256 _milestoneIndex)
        external
        view
        returns (string memory description, string memory proofCID, bool approved)
    {
        Report storage report = reports[_fundId][_milestoneIndex];
