...
contract IPFSVerifier {
     ...
     */
    function getDocumentDetails(string memory _cid)
        external
        view
        returns (address submitter, uint256 timestamp, bool verified)
    {
        return (documentSubmitters[_cid], documentTimestamps[_cid], validDocumentHashes[_cid]);
    }
}
